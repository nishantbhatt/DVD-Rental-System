...
class StandardSession implements sSession {
	...
	protected void _buy(String title, int quantity, boolean limit_check) {
		...
		/* add to transaction history */
		try {
			transactions.add(new DVDTransaction(TransactionID.BUY, title,
					quantity, DVDStatus.SALE, cdvd.getPrice()));
		} catch (TransactionFormatException e) {
			throw new IllegalArgumentException(e.getMessage());
		}
	}
	...
	public void writeTransactionFile() throws IOException {
		...
		FileWriter transac_writer = new FileWriter(transac_file);
		try {
			Iterator<DVDTransaction> transac_iterator = transactions.iterator();
			while (transac_iterator.hasNext())
				transac_writer.write(transac_iterator.next().toString() + "\n");
		} catch (Exception ex) {
			transac_file.delete();
