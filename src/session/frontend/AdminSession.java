...
class AdminSession extends StandardSession implements pSession {
	...
	public void add(String title, int quantity) {
		...
		/* add to transaction history */
		try {
			transactions.add(new DVDTransaction(TransactionID.ADD, title,
					quantity, cdvd.getStatus(), cdvd.getPrice()));
		} catch (TransactionFormatException e) {
			throw new IllegalArgumentException(e.getMessage());
		}
	}
	...
	public void sell(String title, double price) {
		...
		if (dvdInfo.getStatus() == DVDStatus.SALE)
			throw new IllegalArgumentException(
					SessionErrors.DVD_ALREADY_ON_SALE);
		try {
		...
		/* add to transaction history */
		try {
			transactions.add(new DVDTransaction(TransactionID.SELL, title,
					dvdInfo.getCount(), DVDStatus.SALE, dvdInfo.getPrice()));
		} catch (TransactionFormatException e) {
			throw new IllegalArgumentException(e.getMessage());
		}
	}
	...
	public void writeTransactionFile() throws IOException {
		...
		while (transac_iter.hasNext()) {
			...
			if (removed_dvds.contains(tr.getDvd_title())
					&& tr.getTrans_id() != TransactionID.REMOVE)
				transac_iter.remove();
		}
	...
	public void rent(String title, int quantity) {
		...
		/*
		 * making sure that rent transaction is denied if renting from added
		 * DVDs
		 */
		if (addedCount != null) {
